class Solution {
    ...
    public static double[] calcEquation(List<List<String>> e, double[] v, List<List<String>> q) {
        ...
        for (List i : e) {
            map.putIfAbsent((String) i.get(0), new HashMap<String, Double>());
            map.putIfAbsent((String) i.get(1), new HashMap<String, Double>());
        }
        ...
        while (!q.isEmpty()) {
            ...
            for (String nbrs : map.get(re.s1).keySet()) {
                if (!visited.contains(nbrs)) {
                    ...
                    n1.s2 = dis;
                    q.add(n1); // Corrected node addition
                }
            }
        }
